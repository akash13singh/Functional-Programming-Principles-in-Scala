...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    val groups = w.groupBy((element:Char)=>element.toLower)
    (for((k,v)<-groups) yield (k,v.length)).toList.sortBy(_._1)
  }
   ...
   *      List(('a', 2)),
   *      List(('b', 1)),
   *      Liswhat to study before investing in stockst(('a', 1), ('b', 1)),
   *      List(('a', 2), ('b', 1)),
   *      List(('b', 2)),
   *      List(('a', 1), ('b', 2)),
   *      List(('a', 2), ('b', 2))
  ...
  def combinations1(occurrences: Occurrences): List[Occurrences] = {
    print(occurrences)
    val l = scala.collection.mutable.Set[(Char,Int)]()
  ...
  def powerset(set:List[(Char,Int)]):List[List[(Char,Int)]] = {
    set match {
      case Nil => List(set)
      case x::xs => ({for(y <- set ) yield (List(x,y))}::: powerset(xs)).toSet.toList
    }
